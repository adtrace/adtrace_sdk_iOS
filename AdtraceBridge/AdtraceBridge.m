...
- (void)loadWKWebViewBridge:(WKWebView *)wkWebView
    ...
    [self.bridgeRegister registerHandler:@"adtrace_appTrackingAuthorizationStatus" handler:^(id data, WVJBResponseCallback responseCallback) {
        if (responseCallback == nil) {
            return;
        }
    ...
    [self.bridgeRegister registerHandler:@"adtrace_trackMeasurementConsent" handler:^(id data, WVJBResponseCallback responseCallback) {
        if (![data isKindOfClass:[NSNumber class]]) {
            return;
        }
